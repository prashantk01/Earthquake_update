...
public class Query_Util {
    ...
    /*       private static final String SAMPLE_JSON_RESPONSE_URL=
             ...
             "https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=2019-01-01&endtime=2019-02-10&limit=100&minmagnitude=3.0";
           ...
           public Query_Util (Context context){
         mContext=context;
     }
     */
    // Create an empty ArrayList that we can start adding earthquakes to
    public static List<Earthquake> fetchEarthquakeData(String SAMPLE_JSON_RESPONSE_URL) {
    ...
    private static List<Earthquake> extractEarthquakes(String JSON_RESPONSE_URL) {
        if (TextUtils.isEmpty(JSON_RESPONSE_URL))
            return null;
        ...
        try {
            ...
            // build up a list of Earthquake objects  the corresponding data.
            JSONObject jsonObject = new JSONObject(JSON_RESPONSE_URL);
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Earthquake newEarthquakeObj = new Earthquake(mag, loc, time, url);
                earthquakes.add(newEarthquakeObj);
    ...
    private static String makeHttpRequest(URL urlRecieved) throws IOException {
        String jsonResponse = "";
        if (urlRecieved == null)
            return jsonResponse;
        ...
        try {
            urlConnection = (HttpURLConnection) urlRecieved.openConnection();
    ...
    private static URL createUrl(String sampleJsonResponseUrl) {
        URL urlS = null;
        try {
            urlS = new URL(sampleJsonResponseUrl);
